...
export default function Home() {
  ...
  const SchoolPreview = () => <section className="panel compact-panel"><SectionTitle icon={BookOpen} title="مهام المدرسة" action={<button className="ghost-action" onClick={() => setPage("school")}>عرض الكل <ChevronLeft size={15}/></button>} /><div className="mini-school"><div className="mini-count"><GraduationCap size={20}/><strong>{state.school.length}</strong><span>مواد / مهام</span></div><div><strong>{schoolDone} مكتملة</strong><p>أضف المواد، التسميع، والواجبات من صفحة المدرسة.</p></div></div></section>;
  ...
  return <div className={`planner-shell ${state.settings.theme}`} dir={state.settings.language === "ar" ? "rtl" : "ltr"}><aside className="sidebar"><div className="brand"><span className="brand-mark">R</span><div><strong>RONI</strong><small>PLANNER</small></div></div><nav>{navItems.map(item => { const Icon = item.icon; return <button key={item.id} className={page === item.id ? "active" : ""} onClick={() => setPage(item.id)}><Icon size={18}/><span>{t[item.key as keyof typeof t]}</span></button>; })}</nav><div className="sidebar-bottom"><div className="tiny-progress"><span>تقدم اليوم</span><strong>{completion}%</strong><i><b style={{width: `${completion}%`}}/></i></div><div className="profile-line"><span>{user?.name?.slice(0,1).toUpperCase() || "R"}</span><div><strong>{user?.name || "RONI Planner"}</strong><small>{isAuthenticated ? "تمت المزامنة" : "محفوظ على الجهاز"}</small></div></div></div></aside><main className="app-main"><header className="mobile-header"><div className="brand"><span className="brand-mark">R</span><strong>RONI</strong></div><button onClick={() => setPage("settings")}><Settings2 size={19}/></button></header><div className="content-wrap">{renderPage()}</div></main><nav className="mobile-nav">{navItems.map(item => { const Icon = item.icon; return <button key={item.id} className={page === item.id ? "active" : ""} onClick={() => setPage(item.id)}><Icon size={18}/><span>{t[item.key as keyof typeof t]}</span></button>; })}</nav></div>;
}
