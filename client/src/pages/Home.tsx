...
export default function Home() {
  ...
  const applyHistoryDay = () => {
    const oldDay = state.history.find(day => day.date === selectedHistoryDate);
    if (!oldDay) return;
  ...
  const HistoryPage = () => <div className="page-stack"><section className="page-header"><div><div className="eyebrow"><History size={15}/> مراجعة الأيام</div><h1>🗂️ الجداول القديمة</h1><p>اختر يومًا من التقويم لاسترجاع علامات الصح إلى جدول اليوم.</p></div><strong className="stat-chip">{state.history.length} يوم محفوظ</strong></section><section className="panel history-picker"><label>اختر يومًا قديمًا <select value={selectedHistoryDate} onChange={e => setSelectedHistoryDate(e.target.value)}><option value="">اختر التاريخ</option>{state.history.map(day => <option key={day.id} value={day.date}>{formatAttendanceDate(day.date)}</option>)}</select></label><Button type="button" disabled={!selectedHistoryDate} onClick={applyHistoryDay}><History size={16}/> استرجاع علامات الصح</Button><p>سيتم تحديث حالة المهام والصلوات فقط، ولن يتغير اسم المهمة أو وقتها أو ترتيبها.</p></section>{state.history.length === 0 ? <section className="panel empty-state history-empty"><div className="empty-icon">🗓️</div><h3>لا توجد أيام قديمة بعد</h3><p>سيظهر جدول اليوم هنا تلقائيًا عند بدء يوم جديد.</p></section> : <section className="history-list">{state.history.map(day => { const done = day.tasks.filter(task => task.done).length; const prayers = Object.values(day.prayers).filter(Boolean).length; return <article className="panel history-card" key={day.id}><div className="history-card-header"><div><h2>{formatAttendanceDate(day.date)}</h2><p>{done} من {day.tasks.length} مهمة مكتملة · {prayers} من 5 صلوات</p></div><span className="history-percent">{day.tasks.length ? Math.round((done / day.tasks.length) * 100) : 0}%</span></div><div className="history-task-grid">{day.tasks.map(task => <div className={`history-task ${task.done ? "done" : ""}`} key={task.id}><span>{task.icon}</span><strong>{task.title}</strong><time>{task.time}</time>{task.done && <Check size={14}/>}</div>)}</div></article>; })}</section>}</div>;
